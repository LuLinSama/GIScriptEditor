...
private:
	...
	std::any VisitIncrement(const std::any& ref, bool inv, bool pre) override
	{
		...
		auto expr = std::make_unique<ExprContent>();
		ExprBuilder builder(*expr);
		auto& [type, vc] = *std::get<LValueContext>(ref_value->extra).local;
		auto& [content, iterator] = std::any_cast<VarContent&>(vc);
		auto var_ref = std::get<INode*>(content);
		auto tmp = builder.Add(NodeFactory::GetLocalVariable(graph, type));
		auto n = builder.AddFlow(NodeFactory::SetLocalVariable(graph, type));
		builder.Combine(*ref_value, -1);
		tmp->Connect(*n, 0, 0);
		var_ref->Connect(*n, 1, 1);
